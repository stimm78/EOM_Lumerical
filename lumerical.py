...
def draw_MMI(MODE, material_LN,vtx,b):
    MODE.switchtolayout()
    MODE.addplanarsolid()
    MODE.set('vertices',vtx) # must be done first
    MODE.set('facets',b)
    MODE.set("name","test")
    MODE.set("material", material_LN)
    MODE.set("alpha", 0.5)
...
def draw_wg(MODE, material_LN, material_substrate, h_LN, h_substrate, h_etch, 
    ...
    #Bens addition
def draw_wg_atX(MODE, material_LN, material_substrate, h_LN, h_substrate, h_etch, w_ridge, w_slab, theta, wg_length, x0):
    MODE.switchtolayout()
    draw_substrate(MODE, material_LN, material_substrate, h_LN, h_substrate,
                   h_etch, w_slab, wg_length, x0)
    draw_ridge(MODE, material_LN, h_LN, h_etch, w_ridge, theta, wg_length,
               x0, name='ridge')
    time.sleep(0.1)
    
def add_fine_mesh(MODE, finemesh, h_LN, w_ridge, y_factor=1.1, z_factor=1.1):
    y_span = w_ridge*y_factor
    z_span = h_LN*z_factor
    add_fine_mesh_lowlevel(MODE, finemesh, 0, h_LN/2, y_span, z_span)
    
def delete_wgs():
    MODE.switchtolayout()
    MODE.deleteall()

def add_fine_mesh_lowlevel(MODE, finemesh, y0, z0, y_span, z_span):
...
def add_2D_mode_solver(MODE, meshsize, h_LN, h_substrate, w_slab, wg_length,
    ...
    Y_span = w_slab - 2*pml_margin
    Zmin = -h_substrate + pml_margin
    Zmax = h_LN + h_margin
    ...
    if symmetry == 'TE':
        ...
        MODE.set("y max bc", "Anti-Symmetric")
        MODE.set("y max", 0)
    elif symmetry == 'TM':
        MODE.set("allow symmetry on all boundaries", True)
        MODE.set("y max bc", "Symmetric")
        MODE.set("y max", 0)
...
class modes():
    ...
    def get_from_lumerical(self, MODE, k):
        #Get grid
        y = MODE.getdata("FDE::data::mode"+str(k),"y") 
        z = MODE.getdata("FDE::data::mode"+str(k),"z") 
        ...
        self.E = fields_2D(y, z, Ex, Ey, Ez) 
        self.H = fields_2D(y, z, Hx, Hy, Hz) 
        self.neff = neff
...
class fields_2D():
    ...
    Transversal vector field
    '''
    def __init__(self, y, z, Ax, Ay, Az): 
        self.yy = np.squeeze(y) 
        self.zz = np.squeeze(z) 
        self.x = np.squeeze(Ax)
        self.y = np.squeeze(Ay)
        self.z = np.squeeze(Az)
    ...
    def cross(self, E):
        ...
        Cy = Az*Ex - Ax*Ez
        Cz = Ax*Ey - Ay*Ex
        C = field_2D(self.yy, self.zz, Cx, Cy, Cz) 
        return C
